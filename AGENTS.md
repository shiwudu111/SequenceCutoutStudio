...
## 19. 长时间构建 / 打包任务的阻塞处理规则

后续 Codex 执行构建、打包、压缩、依赖安装、electron-builder、Python import 验证等长流程时，必须优先避免“卡着不动让用户等待”。

必须遵守：

    1. 长流程要先分段验证，不要一开始就把所有步骤串起来盲等。
    2. 任一步骤超过 5-10 分钟没有新输出，必须主动停止等待并检查当前阻塞点。
    3. 遇到 spawn EPERM、权限拒绝、子进程无法启动、ExecutionPolicy 阻止等环境问题时，不要继续等待；应立即说明阻塞点，并按权限规则用 require_escalated 重跑必要命令。
    4. 运行完整 build-internal 脚本前，优先确认 npm build / electron-builder / launcher 编译 / Python import / zip 压缩这些阶段的风险点。
    5. 如果完整脚本预计耗时较长，必须提前说明当前阶段、预期耗时、以及无输出时的中断检查策略。
    6. 如果用户问“卡在哪里”，必须先检查是否仍有相关进程在运行，再明确回答当前最后可见阶段和下一步处理方式。
    7. 长命令返回后，必须在 60 秒内给出结果摘要或下一步状态；禁止在工具已经完成后继续长时间“正在思考”。
    8. 如果还需要整理输出，先发一条明确状态，例如“命令已完成，我正在核对产物和状态”，再继续检查。
    9. 对于完整构建链路，最终回答必须包含成功 / 失败、产物路径、zip 大小、关键验证结果、未完成项。
